...
describe('VeltrixLens', () => {
  let veltrixLens;
  let acct;

  beforeEach(async () => {
    veltrixLens = await deploy('VeltrixLens');
    acct = accounts[0];
  });
  ...
  describe('cTokenMetadata', () => {
    it('is correct for a cErc20', async () => {
      ...
      expect(
        cullTuple(await call(veltrixLens, 'cTokenMetadata', [cErc20._address]))
      ).toEqual(
        {
    ...
    it('is correct for cEth', async () => {
      ...
      expect(
        cullTuple(await call(veltrixLens, 'cTokenMetadata', [cEth._address]))
      ).toEqual({
        borrowRatePerBlock: "0",
  ...
  describe('cTokenMetadataAll', () => {
    it('is correct for a cErc20 and cEther', async () => {
      ...
      let cEth = await makeCToken({kind: 'cether'});
      expect(
        (await call(veltrixLens, 'cTokenMetadataAll', [[cErc20._address, cEth._address]])).map(cullTuple)
      ).toEqual([
        {
          cToken: cErc20._address,
          exchangeRateCurrent: "1000000000000000000",
  ...
  describe('cTokenBalances', () => {
    it('is correct for cERC20', async () => {
      let cErc20 = await makeCToken();
      expect(
        cullTuple(await call(veltrixLens, 'cTokenBalances', [cErc20._address, acct]))
      ).toEqual(
        {
          balanceOf: "0",
    ...
    it('is correct for cETH', async () => {
      ...
      expect(
        cullTuple(await call(veltrixLens, 'cTokenBalances', [cEth._address, acct], {gasPrice: '0'}))
      ).toEqual(
        {
          balanceOf: "0",
          balanceOfUnderlying: "0",
  ...
  describe('cTokenBalancesAll', () => {
    it('is correct for cEth and cErc20', async () => {
      ...
      expect(
        (await call(veltrixLens, 'cTokenBalancesAll', [[cErc20._address, cEth._address], acct], {gasPrice: '0'})).map(cullTuple)
      ).toEqual([
        {
          balanceOf: "0",
          balanceOfUnderlying: "0",
  ...
  describe('cTokenUnderlyingPrice', () => {
    it('gets correct price for cErc20', async () => {
      let cErc20 = await makeCToken();
      expect(
        cullTuple(await call(veltrixLens, 'cTokenUnderlyingPrice', [cErc20._address]))
      ).toEqual(
        {
          cToken: cErc20._address,
          underlyingPrice: "0",
    ...
    it('gets correct price for cEth', async () => {
      let cEth = await makeCToken({kind: 'cether'});
      expect(
        cullTuple(await call(veltrixLens, 'cTokenUnderlyingPrice', [cEth._address]))
      ).toEqual(
        {
          cToken: cEth._address,
          underlyingPrice: "0",
  ...
  describe('cTokenUnderlyingPriceAll', () => {
    it('gets correct price for both', async () => {
      ...
      let cEth = await makeCToken({kind: 'cether'});
      expect(
        (await call(veltrixLens, 'cTokenUnderlyingPriceAll', [[cErc20._address, cEth._address]])).map(cullTuple)
      ).toEqual([
        {
          cToken: cErc20._address,
          underlyingPrice: "0",
  ...
  describe('getAccountLimits', () => {
    it('gets correct values', async () => {
      ...
      expect(
        cullTuple(await call(veltrixLens, 'getAccountLimits', [comptroller._address, acct]))
      ).toEqual({
        liquidity: "0",
        markets: [],
        shortfall: "0"
  ...
  describe('governance', () => {
    ...
    describe('getGovReceipts', () => {
      it('gets correct values', async () => {
        expect(
          (await call(veltrixLens, 'getGovReceipts', [gov._address, acct, [proposalId]])).map(cullTuple)
        ).toEqual([
          {
            hasVoted: false,
    ...
    describe('getGovProposals', () => {
      it('gets correct values', async () => {
        expect(
          (await call(veltrixLens, 'getGovProposals', [gov._address, [proposalId]])).map(cullTuple)
        ).toEqual([
          {
            againstVotes: "0",
            calldatas: callDatas,
  ...
  describe('comp', () => {
    ...
    describe('getCompBalanceMetadata', () => {
      it('gets correct values', async () => {
        expect(
          cullTuple(await call(veltrixLens, 'getCompBalanceMetadata', [comp._address, acct]))
        ).toEqual({
    ...
    describe('getCompBalanceMetadataExt', () => {
      it('gets correct values', async () => {
        ...
        expect(
          cullTuple(await call(veltrixLens, 'getCompBalanceMetadataExt', [comp._address, comptroller._address, acct]))
        ).toEqual({
          balance: "10000000000000000000000000",
          delegate: "0x0000000000000000000000000000000000000000",
          votes: "0",
    ...
    describe('getCompVotes', () => {
      it('gets correct values', async () => {
        expect(
          (await call(veltrixLens, 'getCompVotes', [comp._address, acct, [currentBlock, currentBlock - 1]])).map(cullTuple)
        ).toEqual([
          {
      ...
      it('reverts on future value', async () => {
        await expect(
          call(veltrixLens, 'getCompVotes', [comp._address, acct, [currentBlock + 1]])
